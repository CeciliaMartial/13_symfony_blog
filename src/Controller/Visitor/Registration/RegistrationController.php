...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('visitor_registration_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('coucoutoi@patatas.com', 'Céci Chouette'))
                    ->to($user->getEmail())
                    ->subject('Confirmation d\'inscription blog Cecishouette')
    ...
    public function verifyUserEmail(Request $request, TranslatorInterface $translator, UserRepository $userRepository): Response
    {
        ...
        $this->addFlash('success', 'Votre email a bien été verifié. Vous pouvez vous connecter.');
